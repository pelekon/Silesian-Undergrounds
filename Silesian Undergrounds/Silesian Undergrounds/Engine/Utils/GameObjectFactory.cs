...
namespace Silesian_Undergrounds.Engine.Utils
{
    public sealed class GameObjectFactory
    {
        ...
        public static List<PickableItem> ScenePickableItemsFactory(List<GameObject> positionSources)
        {
            ...
            list.Add(FoodFactory(random, positionSources[0].position, positionSources[0].size, isBuyable: true));
            list.Add(KeyFactory(positionSources[1].position, positionSources[1].size, isBuyable: true));
            list.Add(HeartFactory(positionSources[2].position, positionSources[2].size, isBuyable: true));
            ...
            foreach (var source in positionSources.GetRange(3, positionSources.Count - buyableItemsCountType))
            {
                ...
                switch(itemType)
                {
                    case PickableEnum.Ore:
                        list.Add(OreFactory(random, source.position, source.size));
                        break;
                    case PickableEnum.Chest:
                        list.Add(ChestFactory(source.position, source.size));
                        break;
                    case PickableEnum.Food:
                        list.Add(FoodFactory(random, source.position, source.size));
                        break;
        ...
        public static Ore OreFactory(Random rng, Vector2 position, Vector2 size)
        {
            ...
            return new Ore(TextureMgr.Instance.GetTexture(textureName), position, size / 2, 3, null, type);
        }

        public static Food FoodFactory(Random rng, Vector2 position, Vector2 size, bool isBuyable = false)
        {
            ...
            return new Food(TextureMgr.Instance.GetTexture(textureName), position, size / 2, 3, null, type, isBuyable: isBuyable);
        }

        public static Chest ChestFactory(Vector2 position, Vector2 size, bool isBuyable = false)
        {
            return new Chest(TextureMgr.Instance.GetTexture("Items/Chests/chest_1"), position, size, 3, null, isBuyable: isBuyable);
        }

        public static Key KeyFactory(Vector2 position, Vector2 size, bool isBuyable = false)
        {
            return new Key(TextureMgr.Instance.GetTexture("Items/Keys/key_1"), position, size, 3, null, isBuyable: isBuyable);
        }

        public static Heart HeartFactory(Vector2 position, Vector2 size, bool isBuyable = false)
        {
            return new Heart(TextureMgr.Instance.GetTexture("Items/Heart/heart_1"), position, size, 3, null, isBuyable: isBuyable);
        }
    }
}
