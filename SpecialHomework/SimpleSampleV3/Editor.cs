...
namespace SimpleSampleV3
{
    ...
    public partial class Editor : Form
    {
        ...
        public enum CreateMode { None, Walls, Objects, Decorations };
        ...
        const string objectsNamespace = "SimpleSampleV3."; //IMPORTANT: Type the namespace here that all of your classes will be in! Make sure you spell it exactly and put a . at the end!
        ...
        public Editor(Game1 inputGame)
        {
            ...
            mapHeight.Value = game.gameMap.mapHeight;
            mapWidth.Value = game.gameMap.mapWidth;
        }
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            ...
            if (mode == CreateMode.Objects)
            {
                ...
                game.gameObjects.Add(newObject);

                placingItem = true;
                FocusGameWindow();

                SetListBox(game.gameObjects, false);
            }
            else if (mode == CreateMode.Decorations)
            {
                Decoration newDecor = new Decoration();
                newDecor.imagePath = @"Graphics\Other\placeholder_decoration"; //Default image until we load the one we want.
                newDecor.Load(game.Content, newDecor.imagePath);
                game.gameMap.decorations.Add(newDecor);

                placingItem = true;
                SetListBox(game.gameMap.decorations, false);
                FocusGameWindow();   
            }
        }
        ...
        {
            grid = TextureLoader.Load(@"Graphics\Levels\pixel_box", content);
            pixel = TextureLoader.Load(@"Graphics\Other\pixel", content);
        }
        ...
        public void Update(List<GameObject> objects, Map map)
        {
                    ...
                    mode != CreateMode.Walls)
            {
                ...
                else if (Input.MouseLeftClicked() && mode == CreateMode.Decorations)
                {
                ...
                else if (Input.KeyPressed(Microsoft.Xna.Framework.Input.Keys.C) && mode == CreateMode.Decorations)
            ...
            else if (Input.MouseLeftDown() == true && GameWindowFocused() == true)
            {
                ...
                if (mode == CreateMode.Walls)
                {
                    ...
                    if (desiredIndex.X >= 0 && desiredIndex.X < map.mapWidth && desiredIndex.Y >= 0 && desiredIndex.Y < map.mapHeight)
                    {
                        ...
                        if (map.CheckCollision(newWall) == Rectangle.Empty)
                        {
                            ...
                            {
                                oldWall = map.walls[i].boundingBox;

                                if (map.walls[i].boundingBox.Intersects(new Rectangle(newWall.X + map.tileSize, newWall.Y, newWall.Width, newWall.Height))
                                    && map.walls[i].boundingBox.Y == newWall.Y && map.walls[i].boundingBox.Height == newWall.Height)
                                {
                                    ...
                                    map.walls[i].boundingBox = newWall;
                                    break;
                                }
                                else if (map.walls[i].boundingBox.Intersects(new Rectangle(newWall.X - map.tileSize, newWall.Y, newWall.Width, newWall.Height))
                                    && map.walls[i].boundingBox.Y == newWall.Y && map.walls[i].boundingBox.Height == newWall.Height)
                                {
                                ...
                                if (map.walls[i].boundingBox.Intersects(new Rectangle(newWall.X, newWall.Y + map.tileSize, newWall.Width, newWall.Height))
                                    && map.walls[i].boundingBox.X == newWall.X && map.walls[i].boundingBox.Width == newWall.Width)
                                {
                                ...
                                else if (map.walls[i].boundingBox.Intersects(new Rectangle(newWall.X, newWall.Y - map.tileSize, newWall.Width, newWall.Height))
                                    && map.walls[i].boundingBox.X == newWall.X && map.walls[i].boundingBox.Width == newWall.Width)
                                {
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < map.walls.Count; i++)
                            {
                                if (map.walls[i].boundingBox.Intersects(newWall))
                                {
                                    listBox.SelectedIndex = i;
                                    break;
                                }
                ...
                {
                    game.gameObjects[game.gameObjects.Count - 1].startPosition = game.gameObjects[game.gameObjects.Count - 1].position;
                    game.gameObjects[game.gameObjects.Count - 1].Initialize();
                    SetListBox(game.gameObjects, false);
                }
                else if (mode == CreateMode.Decorations && placingItem == true)
                    SetListBox(game.gameMap.decorations, false);
            ...
            else if (Input.MouseRightDown() == true && GameWindowFocused() == true)
            {
                if (mode == CreateMode.Walls)
                {
                    ...
                    for (int i = 0; i < game.gameMap.walls.Count; i++)
                    {
                        if (game.gameMap.walls[i].boundingBox.Intersects(input) == true)
                        {
                            RemoveWall(i);
                            break;
                        }
            ...
            else if (placingItem == true)
            {
                ...
                if (mode == CreateMode.Objects)
                    game.gameObjects[game.gameObjects.Count - 1].position = new Vector2(desiredIndex.X * map.tileSize, desiredIndex.Y * map.tileSize);
                else if (mode == CreateMode.Decorations)
                    game.gameMap.decorations[game.gameMap.decorations.Count - 1].position = new Vector2(desiredIndex.X * map.tileSize, desiredIndex.Y * map.tileSize);
            }

            //Update editor movement if we're paused:
            if (paused.Checked == false && game.gameObjects.Count > 0) //Constantly sync camera position with the player.
                cameraPosition = game.gameObjects[0].position;
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            for (int x = 0; x < game.gameMap.mapWidth; x++)
            {
                for (int y = 0; y < game.gameMap.mapHeight; y++)
                    spriteBatch.Draw(grid, new Vector2(x, y) * game.gameMap.tileSize, null, Color.Cyan, 0f, Vector2.Zero, 1f, SpriteEffects.None, .1f);
            }
        }
        ...
        private void DrawSelectedItem(SpriteBatch spriteBatch)
        {
            ...
            if (mode == CreateMode.Walls)
            {
                if (game.gameMap.walls.Count == 0 || listBox.SelectedIndex >= game.gameMap.walls.Count)
                    return;

                Wall selectedWall = game.gameMap.walls[listBox.SelectedIndex];
                spriteBatch.Draw(pixel, new Vector2((int)selectedWall.boundingBox.X, (int)selectedWall.boundingBox.Y), selectedWall.boundingBox, Color.SkyBlue, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0);
            }
            else if (mode == CreateMode.Objects)
            {
                if (game.gameObjects.Count == 0 || listBox.SelectedIndex >= game.gameObjects.Count)
                    return;

                GameObject selectedObject = game.gameObjects[listBox.SelectedIndex];
            ...
            {
                if (game.gameMap.decorations.Count == 0 || listBox.SelectedIndex >= game.gameMap.decorations.Count)
                    return;

                Decoration selectedDecor = game.gameMap.decorations[listBox.SelectedIndex];
        ...
        private void CopyDecor()
        {
            ...
            Decoration selectedDecor = (Decoration)game.gameMap.decorations[listBox.SelectedIndex];
            Decoration newDecor = new Decoration(selectedDecor.position, selectedDecor.imagePath, selectedDecor.layerDepth);
        ...
        public void RemoveWall(int index)
        {
            ...
            game.gameMap.walls.RemoveAt(index);

            SetListBox(game.gameMap.walls, false);
        }
        ...
        private void ResetGame()
        {
            ...
            {
                game.gameObjects[i].Initialize();
                game.gameObjects[i].SetToDefaultPosition();
            }

            //Reset the decor:
            for (int i = 0; i < game.gameMap.decorations.Count; i++)
                game.gameMap.decorations[i].Initialize();
        }
        ...
        private void LoadLevelContent()
        {
            //Load decor images:
            for (int i = 0; i < game.gameMap.decorations.Count; i++)
                game.gameMap.decorations[i].Load(game.Content, game.gameMap.decorations[i].imagePath);

            //Load objects:
            for (int i = 0; i < game.gameObjects.Count; i++)
            {
                game.gameObjects[i].Initialize();
                game.gameObjects[i].Load(game.Content);
            }
        }
        #endregion
        ...
        public void RefreshListBox<T>(List<T> inputList)
        {
            ...
            string displayMember = "";
                ...
                if (mode == CreateMode.Walls)
                {
                    if (bookmarkIndex == -1 && game.gameMap.walls.Count > 0)
                        bookmarkIndex = 0;
                }
                else if (mode == CreateMode.Objects)
                {
                    if (bookmarkIndex == -1 && game.gameObjects.Count > 0)
                        bookmarkIndex = 0;
                }
                else if (mode == CreateMode.Decorations)
                {
                    if (bookmarkIndex == -1 && game.gameMap.decorations.Count > 0)
                        bookmarkIndex = 0;
                }
        ...
        private void wallsRadioButton_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                mode = CreateMode.Walls;
                SetListBox(game.gameMap.walls, true);
        ...
        private void objectsRadioButton_CheckedChanged(object sender, EventArgs e)
        {
            if (objectsRadioButton.Checked == true)
            {
                ...
                SetListBox(game.gameObjects, true);
        ...
        private void decorRadioButton_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                mode = CreateMode.Decorations;
                SetListBox(game.gameMap.decorations, true);
        ...
        private void removeButton_Click(object sender, EventArgs e)
        {
            ...
            if (mode == CreateMode.Walls)
            {
                game.gameMap.walls.RemoveAt(listBox.SelectedIndex); //Remove the item selected in the listbox.
                RefreshListBox(game.gameMap.walls);
            }
            else if (mode == CreateMode.Objects && game.gameObjects[listBox.SelectedIndex] is Player == false) //Don't remove the player!
            {
                game.gameObjects.RemoveAt(listBox.SelectedIndex);
                RefreshListBox(game.gameObjects);
            }
            else if (mode == CreateMode.Decorations)
            {
                game.gameMap.decorations.RemoveAt(listBox.SelectedIndex);
                RefreshListBox(game.gameMap.decorations);
            }
        ...
        private void listBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (mode == CreateMode.Walls)
            {
                Rectangle selectedWall = game.gameMap.walls[listBox.SelectedIndex].boundingBox;
        ...
        private void xPosition_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (mode == CreateMode.Walls)
            {
                ...
                game.gameMap.walls[listBox.SelectedIndex].boundingBox = selectedWall;
            }
            else if (mode == CreateMode.Objects)
                game.gameObjects[listBox.SelectedIndex].startPosition.X = (float)xPosition.Value;
            else if (mode == CreateMode.Decorations)
                game.gameMap.decorations[listBox.SelectedIndex].position.X = (float)xPosition.Value;
        }
        ...
        private void yPosition_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (mode == CreateMode.Walls)
            {
                ...
                game.gameObjects[listBox.SelectedIndex].startPosition.Y = (float)yPosition.Value;
            else if (mode == CreateMode.Decorations)
                game.gameMap.decorations[listBox.SelectedIndex].position.Y = (float)yPosition.Value;
        }
        ...
        private void layerDepth_ValueChanged(object sender, EventArgs e)
        {
            ...
            game.gameMap.decorations[listBox.SelectedIndex].layerDepth = (float)layerDepth.Value;
        }
        ...
        private void loadImageButton_Click(object sender, EventArgs e) //Browse for an image and then load it in as a decor image...
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    game.gameMap.decorations[listBox.SelectedIndex].SetImage(newImage, fileName);
        ...
        private void mapWidth_ValueChanged(object sender, EventArgs e)
        {
            ...
            game.gameMap.mapWidth = (int)mapWidth.Value;
        }
        ...
        private void mapHeight_ValueChanged(object sender, EventArgs e)
        {
            ...
            game.gameMap.mapHeight = (int)mapHeight.Value;
        }
        ...
        private void newToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            game.gameMap.walls.Clear();
            game.gameMap.decorations.Clear();

            //Add one initial player object:
            game.gameObjects.Add(new Player(Vector2.Zero));

            //Set default editor values:
            mapWidth.Value = game.gameMap.mapWidth = 30;
            mapHeight.Value = game.gameMap.mapHeight = 17;
            savePath = "";
        ...
        private void decorSourceX_ValueChanged(object sender, EventArgs e)
        {
            ...
            game.gameMap.decorations[listBox.SelectedIndex].sourceRect.X = (int)decorSourceX.Value;
        }
        ...
        private void decorSourceY_ValueChanged(object sender, EventArgs e)
        {
            ...
            game.gameMap.decorations[listBox.SelectedIndex].sourceRect.Y = (int)decorSourceY.Value;
        }
        ...
        private void decorSourceWidth_ValueChanged(object sender, EventArgs e)
        {
            ...
            game.gameMap.decorations[listBox.SelectedIndex].sourceRect.Width = (int)decorSourceWidth.Value;
        }
        ...
        private void decorSourceHeight_ValueChanged(object sender, EventArgs e)
        {
            ...
            game.gameMap.decorations[listBox.SelectedIndex].sourceRect.Height = (int)decorSourceHeight.Value;
        }
        ...
        private void SaveLevel()
        {
            ...
            LevelData levelData = new LevelData()
            {
                ...
                walls = game.gameMap.walls,
                decorations = game.gameMap.decorations,
                mapWidth = game.gameMap.mapWidth,
                mapHeight = game.gameMap.mapHeight,
            };
        ...
        public void OpenLevel()
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    game.gameObjects = levelData.gameObjects;
                    game.gameMap.walls = levelData.walls;
                    game.gameMap.decorations = levelData.decorations;
                    mapWidth.Value = game.gameMap.mapWidth = levelData.mapWidth;
                    mapHeight.Value = game.gameMap.mapHeight = levelData.mapHeight;
                    ...
                    if (game.gameObjects.Count > 0)
                        Camera.LookAt(game.gameObjects[0].position);
